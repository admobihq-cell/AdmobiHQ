...
const driverWebEnvPath = resolve(appRoot, "../driver-web/.env.local")
...
function writeMappedEnv(sourceVars, header) {
  ...
  for (const [expoKey, sourceKey] of mappings) {
    ...
    if (expoKey === "EXPO_PUBLIC_API_URL" || expoKey === "EXPO_PUBLIC_DRIVER_URL") {
...
try {
  ...
  console.log("[driver-mobile env:pull] Pulled from Infisical")
} catch {
  const fallbackPath = existsSync(driverWebEnvPath) ? driverWebEnvPath : apiEnvPath
    ...
    writeMappedEnv(
      {
        NEXT_PUBLIC_API_URL: detectLanIp()
          ...
          : "http://localhost:3003",
        NEXT_PUBLIC_DRIVER_URL: "http://localhost:3004",
      },
...
if (additions && existsSync(envPath)) {
  ...
  console.log("[driver-mobile env:pull] Mapped NEXT_PUBLIC_* → EXPO_PUBLIC_*")
}

...
try {
  ...
  console.log("[app-mobile env:pull] Pulled from Infisical")
} catch {
  ...
  if (existsSync(fallbackPath)) {
    ...
    console.log(
      `[app-mobile env:pull] Infisical unavailable — mapped from ${fallbackPath}`,
    )
  } else {
    writeMappedEnv(
      {
      ...
      "# Local defaults — run npm run env:pull -w app first when Infisical is available",
    )
    console.log("[app-mobile env:pull] Wrote local defaults (no Clerk)")
  }
}
...
if (additions && existsSync(envPath)) {
  ...
  console.log("[app-mobile env:pull] Mapped NEXT_PUBLIC_* → EXPO_PUBLIC_*")
}

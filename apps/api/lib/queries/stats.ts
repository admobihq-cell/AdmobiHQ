  ...
  const result = await pool.query<OverviewRow>(
    ...
    lead_rows AS (
      ...
      WHERE deleted_at IS NULL
        AND (bounds.start IS NULL OR created_at >= bounds.start)
    ),
    driver_rows AS (
    ...
    SELECT
      ...
      (SELECT COUNT(*)::text FROM waitlist_entries, bounds
        WHERE deleted_at IS NULL
          AND (bounds.start IS NULL OR created_at >= bounds.start)) AS waitlist,
      (SELECT COUNT(*)::text FROM media_kit_requests, bounds
        WHERE deleted_at IS NULL
          AND (bounds.start IS NULL OR created_at >= bounds.start)) AS media_kit,

 ...
 * useUserIfEnabled — useAuth() must never run unless ClerkProvider is mounted
 * (app/layout.tsx only mounts it when this same flag is on). CI / auth-off
 * builds prerender without ClerkProvider; calling useAuth() there fails the
 * Next.js build.
 */

...
Plan for scaffolding the two driver-facing surfaces from [ROADMAP.md](./ROADMAP.md) §3: **`apps/driver-web`** at `driver.admobihq.com` and **`apps/driver-mobile`** as a new Expo app. When the apps land, this doc splits into `DRIVER.md` / `DRIVER-MOBILE.md` mirroring [APP.md](./APP.md) / [APP-MOBILE.md](./APP-MOBILE.md).
...
Clone `apps/customer-mobile` — expo-router tabs, MapLibre, Sentry RN, EAS + OTA pipeline per [MOBILE-BUILDS.md](./MOBILE-BUILDS.md).
...
- **Prisma:** a generic `PlatformFlag` model (`key` PK, `enabled`, `updated_by_email`, `updated_at`) in the shared schema — seeded with one row, `deliveries: false`. Generic so the next flag reuses the table instead of adding a column somewhere.
- **Read:** `GET /v1/public/config` — no auth, rate-limited like the other public routes, returns `{ flags: { deliveries: boolean } }`. Safe to be public: flags are visibility toggles, never secrets. Next.js apps read it server-side with short revalidation (~60 s); Expo apps fetch on launch and on foreground, so a toggle propagates in about a minute without any OTA update.
- **Write:** `GET/PATCH /v1/flags` — Clerk ops JWT, following the existing CRUD pattern, writing an audit event on every toggle (`ops toggled deliveries on`) so the Activity trail shows who flipped it and when.
...
4. **Docs** — after scaffolding, update the apps table in [ARCHITECTURE.md](./ARCHITECTURE.md) §3, the deploy matrix in [DEPLOYMENT.md](./DEPLOYMENT.md), and [DEV-SETUP.md](./DEV-SETUP.md) ports.

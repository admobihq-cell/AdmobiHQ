...
**Leaving an org, and the accept-invitation conflict it resolves:** `POST /v1/customer/org/leave` is self-service — any member (not just an admin) can leave their own org; the sole owner is refused (`409`) until they transfer admin first, mirroring the account-deletion guard. This matters for invite-accept: since every Clerk user gets an org the moment they first authenticate (lazy bootstrap), clicking someone else's invite would otherwise always collide. `POST /v1/customer/org/invitations/accept/[token]` handles the three cases separately:

1. **An untouched solo org** — sole member, still on the auto-generated `"{FirstName}'s Organization"` / `"My Organization"` name, no campaigns, no support cases, no pending invites. Absorbed silently (`isUntouchedSoloOrg()` in [apps/api/lib/advertiser-org.ts](../../apps/api/lib/advertiser-org.ts)); the invitee never sees a confirmation step, because they never knowingly created that workspace.
2. **A solo org with something in it** — `409` carrying `{ reason: "solo_org_conflict", currentOrgName, campaignCount, supportCaseCount }`. Retrying with `{ leaveSoleOrg: true }` deletes it and completes the accept. The counts exist so the confirmation copy can name what gets stranded: campaigns and support cases are **detached** (`org_id = null`), not deleted, and campaign reads are scoped purely by `org_id`, so a detached campaign is unreachable by every advertiser afterwards. Same detach semantics for `POST /v1/customer/org/delete-organization`, whose counts come from `GET /v1/customer/org/deletion-status`.
3. **A real org with teammates** — never auto-resolved; the caller gets a tailored message pointing at Team settings (transfer) or self-service leave.

The email on the invitation must match the caller's Clerk address, and the check **fails closed** — an address Clerk can't resolve is refused, so a leaked token can't be redeemed by whoever holds it. Invites expire after 7 days (`410`). Both `POST /v1/customer/org/members` (which sends mail) and the accept route are rate-limited **per Clerk user id**, not per IP: `checkRateLimit`'s `identifier` option exists for that, because Kenyan mobile carriers NAT many subscribers behind one address.
